<script lang="ts">
  ...
  <div class="max-w-6xl w-full mx-auto mt-6 px-4">
      <div class="flex flex-col items-center space-y-4">
          ...
          <p class="text-5xl font-bold">Můj seznam přání</p>
          <a href={`${window.location.origin}/p/${$authStore?.$id}`} target="_blank" class="text-lg text-blue-600 bg-blue-100 rounded-3xl py-2 px-6">{window.location.origin}/p/{$authStore?.$id}</a>
          ...
          <form on:submit={onSave} class="flex w-full flex-col space-y-3">
            ...
            <input bind:value={name} type="text" placeholder="Matěje" class="w-full rounded-2xl px-8 py-3 bg-white border border-gray-300 text-gray-700 font-semibold text-lg">
            <p class="text-lg font-semibold text-gray-800">Poděkováni na závěr:</p>
            <input bind:value={note} type="text" placeholder="Ďekujúúúú." class="w-full rounded-2xl px-8 py-3 bg-white border border-gray-300 text-gray-700 font-semibold text-lg">
            <p class="text-lg font-semibold text-gray-800">Oblibená barva:</p>
            <input bind:value={color} type="color" class="h-14 w-full rounded-2xl px-8 py-3 bg-white border border-gray-300 text-gray-700 font-semibold text-lg">
...
<div class="border-dashed col-span-12 border-gray-200 bg-white md:col-span-6 border-2 border rounded-3xl w-full p-6">
      <p class="font-bold text-gray-900 text-xl w-full text-center">Váš seznam je prázdny.</p>
</div>
            ...
            <div class="flex justify-center w-full">
              ...
              <button type="button" on:click={() => showNewItem = true} class="mt-4 rounded-2xl px-8 py-3 bg-blue-500 text-white font-semibold text-lg">
                <p>Pridat Predmět</p>  
              </button>
